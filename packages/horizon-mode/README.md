...
It adds budget-aware continuation turns, clean Git checkpoints, and explicit final-submission confirmation. When `sandbox-timer remaining` is available, Horizon mode activates automatically and uses that task's wall-clock budget.
...
While active, the agent receives the remaining budget and is prompted to continue measuring, improving, and validating until it confirms a final checkpoint. The mod registers one tool:
...
The mod writes only this state file. `submit` invokes fixed `git` commands in the active workspace. The mod does not use a shell, network access, or secrets.
